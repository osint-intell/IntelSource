...
class DNSCollector:
    ...
    def __init__(self, timeout: int = 5, retries: int = 2, rate_limit_delay: float = 0.1):
    ...
    def lookup(self, domain: str, record_types: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        for record_type in record_types:
            ...
            dns_data['records'][record_type] = self._query_record(domain, record_type)
    ...
    def reverse_lookup(self, ip_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Reverse DNS lookup failed for {ip_address}: {str(e)}")
            return {
                'ip_address': ip_address,
                'hostnames': [],
                'status': 'failed',
