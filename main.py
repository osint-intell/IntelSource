    ...
    def run(self):
        ...
        while True:
            try:
                self.show_menu()
                choice = Prompt.ask(
                    "[bold cyan]➜[/bold cyan] Enter your choice",
                    choices=[
                        "1",
                        "2",
                        "3",
...
╚═══════════════════════════════════════════════════════════════════╝[/bold cyan]
        ...
        console.print(
            "[dim]Gathering intelligence across multiple sources...\n[/dim]")

    def show_menu(self):
        """Display main menu with Rich Table."""
        menu_table = Table(
            title="[bold magenta]IntelSource - Main Menu[/bold magenta]",
            show_header=False,
            box=None)
    ...
    def menu_ip_lookup(self):
        ...
        console.print(
            Panel(
                "[bold cyan]IP Address Lookup[/bold cyan]",
                expand=False))
        ...
        if not is_valid_ip(ip_address):
            console.print(
                f"[bold red]✗ Invalid IP address: {ip_address}[/bold red]")
            return
    ...
    def menu_email_validation(self):
        ...
        console.print(
            Panel(
                "[bold cyan]Email Validation[/bold cyan]",
                expand=False))
        ...
        if not is_valid_email(email):
            console.print(
                f"[bold red]✗ Invalid email address: {email}[/bold red]")
            return
    ...
    def menu_full_reconnaissance(self):
        ...
        console.print(
            Panel(
                "[bold cyan]Full Reconnaissance[/bold cyan]",
                expand=False))

        target = Prompt.ask(
            "[bold]Enter target (IP, domain, or email)[/bold]").strip()
        target_type, is_valid = classify_target(target)

        if not is_valid:
            console.print(
                f"[bold red]✗ Invalid target format: {target}[/bold red]")
            return
        ...
        with console.status(f"[bold green]⟳ Performing full reconnaissance on {target}...[/bold green]"):
            ...
            try:
                if target_type == 'ip':
                    ...
                    result['reverse_dns'] = self.dns_collector.reverse_lookup(
                        target)
                    ...
                    result['email_validation'] = self.email_validator.validate(
                        target)
                    if '@' in target:
                ...
                console.print(
                    f"[bold red]✗ Reconnaissance failed: {e}[/bold red]")
    ...
    def menu_bulk_import(self):
        ...
        console.print(
            Panel(
                "[bold cyan]Bulk Import[/bold cyan]",
                expand=False))

        file_path = Prompt.ask(
            "[bold]Enter file path (one target per line)[/bold]").strip()
        ...
        try:
            ...
            with console.status(f"[bold green]⟳ Processing {len(targets)} targets...[/bold green]"):
                for i, target in enumerate(targets, 1):
                    ...
                    try:
                        if target_type == 'ip':
                            result['geoip'] = self.geoip_collector.lookup(
                                target)
                        elif target_type == 'domain':
                            result['whois'] = self.whois_collector.lookup(
                                target)
                            result['dns'] = self.dns_collector.lookup(target)
                        elif target_type == 'email':
                            result['email_validation'] = self.email_validator.validate(
            ...
            console.print(
                f"[bold green]✓ Processed {len(results)}/{len(targets)} targets[/bold green]")
    ...
    def _display_results(self, target: str, result: Dict[str, Any]):
        ...
        console.print(
            Panel(
                f"[bold cyan]Results for {target}[/bold cyan]",
                expand=False,
                style="cyan"))
    ...
    def save_result(self, result: Dict[str, Any]):
        ...
        try:
            ...
            console.print(
                f"[bold green]✓ Results saved to: [cyan]{filepath}[/cyan][/bold green]")

        except Exception as e:
            console.print(
                f"[bold yellow]⚠ Failed to save results: {e}[/bold yellow]")
...
def main():
    ...
    try:
        if not os.path.exists('.env'):
            console.print(
                "[bold yellow]⚠ Note: .env file not found.[/bold yellow]")
            console.print(
                "[dim]Copy .env.example to .env and add your ipinfo.io token for full functionality.[/dim]\n")
